...
import { adminMenuItems, profileMenuItems } from '../models/Menu'
...
export default {
  ...
  beforeMount () {
    AppModule.UPDATE_MAIN_LAYOUT_TYPE('vertical')
  },
  created () {
    const color = this.navbarColor === '#fff' && this.isThemeDark ? '#10163a' : this.navbarColor
    this.updateNavbarColor(color)
    this.setNavMenuVisibility(AppModule.mainLayoutType)
    this.navMenuItems = this.$route.meta.rule === 'admin' ? adminMenuItems
      : this.$route.meta.rule === 'customer' ? profileMenuItems : AppModule.navMenuItems
  },
  methods: {
    changeRouteTitle (title) {
      this.routeTitle = titleprofileMenuItems
    },
